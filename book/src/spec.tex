...
\eei{.bib} files with their own syntax. Single bibliographic entry is of the
form
\begin{lscommand}
...
\end{description}
Note that the above list is far from exahaustive both in terms of \carg{entry
...
\end{lscommand}
command. The \carg{options} are comma delimited key vaule pairs that allow to
customize the behaviour of biblatex package. Some of them will be explained
later.
...
After executing the procedure on the following file we may apprecieate the
result
...
was a time traveller~\cite{dream}.
\ldots
