...
\begin{lscommand}
  \csi{verbatiminput}[filename: m]
\end{lscommand}
command, which allows you to include raw ASCII text into your
...
\csi{ref} and \csi{pageref} become hyperlinks. Additionally, the table
...
Just add \mintinline{latex}|\usepackage{hyperref}| as the \emph{last} command into
the preamble of your document.

Many options are available to customise the behaviour of the
...
\begin{FlushLeft}
  \begin{description}
    ...
    \item [\texttt{pdftitle (=\{text\})}] defines the title that gets
          displayed in the \enquote{Document Info} window of Acrobat
...
use the \csi{href} command without the \enquote*{\texttt{http://}} bit:
\begin{minted}{latex}
...
email messages by using the \csi{href} command inside the \csi{author}
...
link
\begin{code}
  \mintinline{latex}|\href{mailto:mary@oetiker.ch}{Mary Oetiker}|
\end{code}
would
...
! pdfTeX warning (ext4): destination with the same
  ...
  duplicate ignored
\end{verbatim}
appear when a counter gets reinitialised, for example by using
the command \csi{mainmatter} provided by the \texttt{book} document class. It
...
\begin{lscommand}
  \csi{texorpdfstring}[\bs TeX{} text: m, Bookmark Text: m]
\end{lscommand}
...
\end{minted}
\end{code}
which turns \mintinline{latex}|\section{$E=mc^2$}| to ``E = mc ** 2'' in the bookmark area.

If you write your document in Unicode and use the \cargv{unicode} option for
...
well, so \mintinline{latex}|\textit| and \mintinline{latex}|\textbf| will work as usual. When the
...
distributed with \TeX{}Live~2010\footnote{The behaviour has changed with this
...
\begin{code}
  \url{.../texmf-dist/fonts/opentype},
\end{code}
just install them in your operating
