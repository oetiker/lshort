...
\texttt{\}}
\end{lscommand}
The fields required dependend on the bibliography style you choose,
...
Here is a complete example to show how the bits work together. Iin this and all the following examples, the \texttt{example.bib} file is assumed to be the same as in~\ref{fig:bibfile}.
...
entries in chronological order simply pass the \carg{ynt} option. Other options
allow sorting by citation order (\carg{none}) and by the number of citations
(\carg{count}).
\begin{example}[standalone,
  template=empty,
  biber,
...
While the default citation style is \carg{numeric}, this can be easily changed by
setting the preferred style via the \carg{style} option. Choose from
\carg{alphabetic},
\carg{authoryear}, \carg{authortitle} or \carg{verbose}. Some styles come in
several variations.
\begin{example}[standalone,
  template=empty,
  biber,
...
use the \ci{nocite}[marker] command. It will insert invisible citations, thus
...
them will be printed; instead ``et al.'' shows up at the end of the author list. This behavior
...
Most of the citing commands (\ci{cite} included) allows to insert notes
...
the bibliographic entry, and use the information directly in the text. This is includes commands such as \ci{citeauthor},
...
If you want to be less tied to a specific style, the \ci{autocite}
...
this behavior, enclose ``and'' in curly brackets
\begin{verbatim}
@book{
