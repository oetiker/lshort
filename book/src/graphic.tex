...
cleverly placing predefined elemements onto the canvas. A complete
...
quality vector graphics with all current \TeX{} systems including fulll
support for pdf.
...
mathematically sophisticated programming language of \MF but ontrary to \MF,
...
Pgf comes with a high level access language provided by the  \pai{tikz} package.
TikZ provides highly efficient commands to
draw graphics right from inside your document. Use the \ei{tikzpicture}
environment to wrap your TikZ commans.

As mentioned above, there is an excellent manual for PGF and frieds. So
...
\end{tikzpicture}
\end{example}
Not the semicolon (\texttt{;}) character. It separates the individual commands.
...
\end{tikzpicture}
\end{example}
If you are using \pai{tikz} in connection with bable some of the characters used in the
TikZ language may get modified by babel, leading to odd errors. To counter act this, add 
