...
\pai*{siunitx} package you do not need to know most of the rules. If you
consequently use \pai{siunitx} commands then units in your document will be
typeset correctly.
...
typesets quantity, i.e.\ number followed by unit. All of the commands provide
optional \carg{options} argument, which is a comma delimited key value list
that can influence the output of the command. The \carg{unit} argument can be
provided in one of two styles, either \enquote{literal} or
\enquote{interpreted}.
...
Sometimes you may want to make the units use a different color so they stick
out more. This can be done using \cargv{unit-color}. Options
\cargv{number-color} and \cargv{color} may be used to color only the numeric
...
The behaviour of the commands may be customized even further. For a full list
...
a full document.
\begin{figure}[htp]
\begin{example}[standalone, template=empty, paperwidth=6cm, nocache]
\documentclass{article}

\usepackage[paperheight=\height,paperwidth=\width,margin=0.3cm]{geometry} %!hide
\usepackage{booktabs} %!hide
\sloppy %!hide
\usepackage[
...
\subsection{Other \pai{siunitx} commands} \label{sec:arraymat}
...
\qtylist{2;5;7}{\L}.\\
Acceptable range is
range \qtyrange{1}{10}{\kg}. \\
Area is \qtyproduct{2x5}{\metre}.
\end{example}
...
\ci{DeclareSIQualifier} allos definition of macros in case additional are
needed.
\begin{example}[examplewidth=0.4\linewidth]
\DeclareSIUnit{\pt}{pt}
...
package adds the special column specifier \cargv{S} for the \ei{tabular}
...
All the numbers in such columns are automatically parsed by \pai{siunitx}. It
is also possible to align the numbers inside a \ci{multicol} column, using the
\ci{tablenum} command.
