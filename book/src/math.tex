...
equation in \ci{[} and \ci{]}:\footnote{\index{equation!amsmath@\textsf{amsmath}}\index{equation!LaTeX@\LaTeX{}}
...
\begin{lscommand}
  \ci*{frac}[numerator:m ! denominator:m]%chktex 26
\end{lscommand}
command.
\begin{example}
\ldots{} And since
...
\ci*{stackrel}[above:m ! below:m] puts the symbol given%chktex 26
...
$\qty{1}{\L} = \qty[
  exponent-product = \cdot]{1e-3}{\cubic\m}$
\end{example}
...
a full document.
\begin{listing}
  \begin{example}[standalone, template=empty, examplewidth=\linewidth,paperwidth=6cm, vertical_mode]
...
\begin{document}
All the units in the document are typeset according to setup
in the preamble: \qty{9.81}{\m\per\s\squared}.
...
In table~\ref{table} custom options are used, but now we
are back to normal: \qty{3.637e-4}{\m\squared\per\s}.
\end{document}
\end{example}
...
Listing~\ref{lst:siunitxS} for an example.
\begin{listing}
  \begin{example}[examplewidth=0.8\linewidth, vertical_mode]
...
Listing~\ref{lst:tableformat} for an example.
\begin{listing}
  \begin{example}[examplewidth=0.7\linewidth, vertical_mode]
\sisetup{exponent-mode = fixed}
\begin{tabular} {
...
\cargv{table-text-alignment} options. See Listing~\ref{lst:salignment} for an example.
\begin{listing}
  \begin{example}[examplewidth=0.7\linewidth, vertical_mode]
\begin{tabular} {
    @{}
...
using the \ci{tablenum} command as seen in Listing~\ref{lst:tablenum}.
\begin{listing}
  \begin{example}[examplewidth=0.7\linewidth, vertical_mode]
...
\end{tabular}
\end{example}
  \caption{An example of using the \ci{tablenum} command.}\label{lst:tablenum}
\end{listing}
