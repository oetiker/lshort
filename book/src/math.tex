...
\begin{enumerate}
  ...
  \ci{qquad} (we'll get back to that later, see \autoref{sec:math-spacing}).
  ...
  \item Each letter is considered to be the name of a variable and will be
        ...
        text using the \ci{text} command (see also \autoref{sec:fontsz}~on
...
using the \ci{tablenum} command as seen in \autoref{lst:tablenum}.
\begin{listing}
  \begin{example}[examplewidth=0.7\linewidth, vertical_mode]
...
\ci{NewDocumentCommand} in \autoref{sec:new_commands} on \autopageref{sec:new_commands}.
\begin{example}[examplewidth=0.35\linewidth]
