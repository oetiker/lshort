...
Use \csi{cdot} to typeset a single centered dot. \csi{cdots} is
three centered \emph{\wi{dots}} while \csi{ldots} sets the dots low (on the
...
don't want anything on the right, use the invisible ``\csi{right.}'':
\begin{example}
\begin{equation*}
1 + \left(\frac{1}{1-x^{2}}
    \right)^3 \qquad 
...
\csi{label}[\ldots] is defined, then this label is passed on
