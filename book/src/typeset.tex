...
Since the turn of the Century most Operating Systems are based on Unicode
(Windows XP, Mac OS X, Linux). Therefore it is recommended to use \iei{utf8}
for any new project. The \iei{utf8} encoding used by \pai{inputenc} only
defines the characters that are actually provided by the fonts used. If you
need more (non-latin) characters have a look at \hologo{XeLaTeX} in section
\ref{sec:xetex} a Unicode based \TeX-engine.
...
The order of loading of these two packages is important, since the
\pai{inputenc} package will only set the encoding for defined characters.
...
\begin{description}
\item[\hologo{XeTeX}] was developed for Mac OS X but is now available for all
