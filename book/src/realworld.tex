...
The package \pai*{polyglossia} is a replacement for venerable \pai{babel}
...
\end{lscommand}
where language is either the name of the language to use such as \cargv{gaelic}
or \cargv{japanese}, or its BCP-47 tag such as \cargv{de-CH} or
...
You can also pass \carg{options} as an optional argument the environment.
...
required in the second language. The default Latin Modern for example does not
contain Cyrillic letters. The solution is to define a font that will be used
for that language. 

To set the fonts we will use the command
\begin{lscommand}
  \ci*{newfontfamily}[familyname:M ! options:o ! font:m]
\end{lscommand}
\noindent from the \pai{fontspec} package\footnote{\pai{fontspec} is loaded by
\pai{polyglossia} so you do not need to load it manually.} that will be
described in much more detail in section~\ref{sec:fontspec}.

When changing text language, polyglossia checks whether a font family named
\cargv{\textit{language}font} exists and changes to it if it's available.

If you are happy with the default font, then for Cyrillic or Greek letters you
may want to try the \enquote{CMU} font which contains some additional glyphs
while looking nearly identical to the Latin Modern.\footnote{Both fonts are
actually based off of Computer Modern which was the original font used by
the first \TeX{} versions.}

If you have CMU font installed in your system (unlikely) or you are using 
...
\texttt{monospace} (used when displaying code). In order to adjust them for
language define \cargv{\textit{language}fontsf} family for sans serif and
\cargv{\textit{language}fonttt} for monospace. For example, to define CMU as a
font for Greek in all three fonts you would write
\begin{verbatim}
\newfontfamily\greekfont{CMU Serif}
...
Above commands redefine the fonts for languages used as other in polyglossia.
If you want to influence the font of the main document language use the
\ci{setmainfont}, \ci{setsansfont} and \ci{setmonofont} that work the same way
but without the \carg{familyname} argument.
\begin{verbatim}
...
Fonts will be explored in more detail in section~\ref{sec:fontspec}.
...
much more limitited than \pai{bidi}.} in order to support RTL languages. The
...
The IranNastaliq font developed by the SCICT and updated by Mohammad Saleh
Souzanchi is available at~\cite{font:IranNastaliq}.
