...
These days, the \hologo{AmSLaTeX} features and some additional ones are provided by the
\pai*{mathtools} package. It loads the \pai{amsmath} package internally, adjusts
some settings, fixes some behaviours and adds some additional commands. We
recommend to load it together with the \pai*{unicode-math} package for an optimal
...
The environment allows to put multiple bits of math on a single line by
...
columns as in the \env{tabular} environment and does not produce any delimiters around
...
The built-up fractions and binomial coefficients are specialized examples of a
