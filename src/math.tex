...
Note the extra \csi{negmedspace} is required to make the
plus sign properly separate from the variable name. The \csi{negmedspace}
command adds a medium negative space which cancles out the space \LaTeX{}
adds between variable and operator in a formula like this \(a+b\), while
filling the role of a variable when \LaTeX{} has to decide whether
the next symbol is a relation symbol or a unary operator.
\begin{example}
\begin{IEEEeqnarray*}{rCl}
a & = & d + e + f \\
& & + g \quad \text{too narrow!}\\
& & \negmedspace{} + g \\
& = & k + 2g
\end{IEEEeqnarray*}
\end{example}
