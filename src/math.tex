...
the box, there are a few areas where it is lacking useful features and some of
the design choices have not aged well over the last 30 years. The
\emph{\wi{American Mathematical Society}} was very interested in \TeX{} from
the earliest days and sponsored the development of several well received
enhancements to improve \LaTeX{} math abilities. The enhancements are known as
\hologo{AmSTeX} and \hologo{AmSLaTeX}.

These days the \hologo{AmSLaTeX} features and more is provided by the
\pai*{mathtools} package which loads \pai{amsmath} package internally, adjusts
some settings and fixes some behaviours and adds some additional commands. We
recommend load it together with the \pai*{unicode-math} package for an optimal
...
\pai{unicode-math} is the some of the older math packages rely on the
classic math font encoding. So if you have to use them, then do not use the
\pai{unicode-math} package to save your sanity.
...
one allows you to customise the the visible label of the equation.
\begin{example}
\begin{equation}
  \tag{Ingsoc's theorem}
  \label{ingsoc}
...
\end{example}
If you want to typeset normal text within a formulae you can use the \csi{text}
command.
\begin{example}
\( 1 + \text{one} = 2\)
\end{example}
...
Exponents are written using the superscript \ai{\^{}}\index{exponent}
...
\enquote{\(\sin\)} name \(s\), \(i\) and \(n\) are not symbols, but a
...
normal sub- or supscript next to them, you can simply surround them with curly
braces.
\begin{example}
\[
  {\sum_{a}}^b
...
more powerful \csi{DeclarePairedDelimiterX} command. It allows you to define
delimiters that are composed of more than one part (such as bra-kets) and
manipulate its arguments. For full description, check out the the documentation
...
line comments with the \csi{text} command in such transformations.
\begin{example}
\begin{align*}
  \sum_{k=0}^n k
    &= \sum_{k=0}^{n-1} k + n
...
functionalities. More information about \texttt{IEEEeqnarray} can found in
...
within the environment, with its usage similar to the \cs{multicol} environment.
\begin{example}
\begin{IEEEeqnarray}{rCl}
...
may be customised even further. For a full list of options check out the
...
environment. Note that there are no delimited variants of it so you must
...
compared to the \csi{lvert}, \csi{rvert} commands. The former is a symbol while
...
In case of matrices you may also need non-horizontal dots. These are accessible
under the commands \csi{vdots} (vertical dots \(\vdots\)), \csi{ddots}
