...
it and the new version, because the \ai{\$} works fine, but it may become an
issue if you want to modify it yourself. Moreover the \csi{(}, \csi{)} commands
...
use one you don't have a choice.
...
These commands are \TeX{} primitives that use nonstandard syntax, which may
lead to weird errors. It is preferable to use the \LaTeX{} equivalents, which
use the standard syntax. The \csi{kern} primitive acts as either a vertical or
a horizontal space, depending on whether it is used, respectively, between
paragraphs or inside one.
\begin{chktexignore}  
