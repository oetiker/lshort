...
\ei{foreigndisplayquote} are also available and work like the ones
...
columns. The commands only required argument is the span of columns
...
environment that works similar to the \ei{tabular} environment, but allows
pagebreaks inside.
...
table rows. One of them is \csi{textbackslash*} which, similar to its
...
define a new environment \mintinline{latex}|cat| that can be used like
the \ei{figure} and \ei{table} environment and \mintinline{latex}|\listofcats| that
works the same as to \csi{listoftables}. You can customise the floats using the
