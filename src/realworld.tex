...
The first thing \LaTeX{} needs to know when processing an
...
\begin{table}
  ...
  \begin{tabular}{@{}>{\RaggedRight}p{2.5cm}p{9cm}@{}}
    ...
    default, while the \cargv{report} and \cargv{book} classes do.\index{title}          \\
    ...
    chapters. By default, the \cargv{report} class starts chapters on
    the next page available, while the \cargv{book} class starts them on
    right hand pages.                                                        \\
    \bottomrule
  \end{tabular}
\end{table}
...
%$ 'section': The next section is about ready-made strings, the next subsection
%$ is about hyphenation. The next paragraph, perhaps?
...
Initial versions of \TeX{} were only intended to support the English language,
...
you are typing in your native language and have all the necessary characters on
your keyboard, but they still come in handy when accented characters
...
output is produced only if the font supports the combination.
...
\index{., space after} %$ Is the above tilde intentionally visible in output?
...
if typed on a typewriter, with all spaces, and without any \LaTeX{} commands
being executed.%$ Should this explicitly state 'in monospaced font'?
\begin{example}
Use \verb|\LaTeX{}| to
print \LaTeX.
...
it together with a \cargv{p\{...\}} column, the space specified will be
%$ The 'space specified', or the 'resulting space'? The latter, surely?
...
page breaks will not move, even when some included files are omitted.%$ or \csi{include}?
