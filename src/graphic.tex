...
To start, some background on how to think about graphics.
...
years such as \pai*{metapost}, \pai*{asymptote} or \pai*{xypic}. Today the most
...
such as \pai*{pgfplots} for plotting functions and presenting data or
...
To use \pai{pgf} and \TikZ{} it is sufficient to put the following line in the
preamble:
\begin{minted}{latex}
  \usepackage{tikz}
\end{minted}
Doing so will provide the \ei{tikzpicture} environment and the \csi{tikz}
...
\end{tikzpicture}
\end{example}
\TikZ{} attempts to be smart about the way it positions the lines between nodes.
...
If you want to create curved lines the simplest way is to use \ltx{to} for
connecting points. It works the same as \ltx{--}, but you can provide an optional arguments with \cargv{in} and \cargv{out} keys to define the angle of
the connection.
\begin{example}[vertical_mode, examplewidth=0.8\linewidth]
...
\ltx{.. controls ..} connection for specifying a Bézier curve with one
...
The lines do not need to be continuous either. You can specify a \cargv{dash
...
drawn, but you can pass \cargv{draw} and \cargv{fill}, possibly set to a
colour, to reveal it.
\begin{example}
\tikz{\node[draw] (0, 0)
  {Some Text};}
...
\pai{pgf} and \TikZ{} do not rely on \LaTeX{}. Infact they can be used with
...
You can perform additional computations on existing
