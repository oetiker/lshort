...
\begin{tikzpicture}
  \draw (0, 0)
    ...
    -- (0, 2);
\end{tikzpicture}
%$ consider maybe (1, 2) as the final point above, since the asymmetry then
%$ allows the reader to identify the origin (0, 0), the direction of drawing,
%$ and the direction of increasing y (since SVG et al have y increasing
%$ downwards). Alternatively, changing the polar point to say (30:3) might
...
%$ 'a properly mitred join'? Impressive example, by the way.
...
%$ 'It' is ambiguous; I think you are referring to the \path command.
an action
%$ 'must' and 'optional' don't go together; some or other action is mandatory?
must be specified as an argument. The \csi{draw} command simply
...
\end{lscommand}
%$ Standard English would be 'coordinates' (above & below), a coordinate being a
%$ single scalar component of a point, but Tantau does say 'coordinate'.
...
argument, with \cargv{in} and \cargv{out} keys, to define the angles of
the connection.%$'terminal angles'?
\begin{example}[vertical_mode, examplewidth=0.8\linewidth]
...
%$ Bézier seems to work (I had no idea). Should we be using B\'ezier, or is that
%$ so thirty years ago?
...
%$ provide the reference again? it's been a while since we last saw it.
...
\cargv{align} key; without it, new lines are ignored. Possible values %$ newlines?
...
coordinate added. %$ 'as the previous point plus this vector'?
\begin{example}
\begin{tikzpicture}[scale=2]
  \draw (0, 0) -- ++(0, 1) --
    ++(1, 0) -- ++(0, -1) --
