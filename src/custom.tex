...
\begin{description}
  ...
  \item[size] The size of the glyphs. For example, \enquote*{\qty{10}{pt}} or
    \enquote*{\qty{12}{pt}}.
\end{description}
\LaTeX{} automatically chooses the appropriate font family, series, shape and size based on the
...
important to note that not every combination of
family\slash{}series\slash{}shape exists as an actual font. \LaTeX{} will complain if you try for something that does not exist.
...
There are only two predefined \LaTeX{} series: medium (\csi{textmd}) and bold (\csi{textbf}).
\begin{example}
\textmd{The default.} \\
\textbf{Bold font.}
\end{example}
...
(\csi{textit}), oblique or slanted\footnote{The difference between oblique and
...
upper-lowercase (\csi{textulc}). These are not actually shapes but they change
the current shape to, respectively switch back to upright font and to disable
small capitals. The command \csi{textnormal} is just the combination of the
two.
\begin{example}
\textsl{\textsc{Back to
  \textup{upright.}}} \\
...
Note! Using explicit font setting commands defy the the basic idea of \LaTeX{} described in \autoref{sec:logical_structure}, which is to separate the logical and visual markup
where the fonts get switched automaticcaly according to the requirements of the context.
A simple rule of thumb:
If you use the same font changing command in several places in order to typeset
a special kind of information, you should use \csi{NewDocumentCommand} to
...
\end{code}
the directory, where the \cargv{...} denotes the path \TeX{}Live was installed to.
...
Latin Modern Font provides a special \enquote{fk} ligature for the Polish language.
You can set the \cargv{Language} key to a given language to enable these
features.
\begin{example}
agrafka
...
In the \autoref{sec:logical_structure} we have used different text colours in
the example. These can be obtained with the \pai*{xcolor} package. It provides
two commands to change the text colour:
\begin{lscommand}
  \csi{color}[model: o, color: m] \\
  \csi{textcolor}[model: o, color: m, text: m]
\end{lscommand}
The \csi{color} is a switch version while \csi{textcolor} only applies to its
argument. If no \carg{model} is specified, then \carg{color} is specified as
colour expression. The simplest colour expression is just the name of the colour,
for example \cargv{yellow} or \cargv{red}.
\begin{example}
\textcolor{yellow}{foo}
...
in degrees, that is a number between \(0\) and \(360\). The \cargv{hue} accepts
...
\end{lscommand}
This makes it possible to use the newly defined colour in colour mixing and such.
\begin{example}
...
\textcolor{MyRed!60}{Tints
  are also available!}
\end{example}
Be careful though, since it doesn't against redefinition. If you want to check
whether you haven't redefined some colour put \csi{tracingcolors} in your
...
So far we have only considered changing text colour. It is however possible to
also
change the background colour of the document page. To do this use the
\begin{lscommand}
  \csi{pagecolor}[model: o, color: m]
\end{lscommand}
command, which arguments are the same as in the \csi{color} command. If you
...
\begin{table}
  ...
  \begin{tabular}{@{}clrrl@{}}
    ...
    \mintinline{latex}{em} & \multicolumn{3}{l}{approx width of an \enquote*{M} in the current font}  & \demowidth{1em}                                               \\
    \mintinline{latex}{ex} & \multicolumn{3}{l}{approx height of an \enquote*{x} in the current font} & \demowidth{1ex}                                               \\
    \bottomrule
  \end{tabular}
\end{table}

The last two units mentioned in the table are relative to the current font.
Historically they were related to the \enquote*{M} and \enquote*{x} glyphs in a
given font but today they are arbitrarily set by fonts. These units are useful
if we want the length to scale proportionally when used with different font
sizes. The \mintinline{latex}|em| unit is usually used for horizontal lengths,
while the \mintinline{latex}|ex| is used for vertical lengths.
\begin{example}
