...
\end{code}
directory, where the \cargv{...} denotes the path \TeX{}Live was installed to.
...
\subsection{Font Colours}
In the \autoref{sec:logical_structure} we have used different font colours in
...
If you prefer substractive colour model, both \cargv{cmy} and \cargv{cmyk} are
available. These both accept decimal numbers between \(0\) and \(1\) to specify
...
If you want to use given colour more than once it probably makes sense to define
...
So far we have only considered changing font colour. It is however possible to
also
