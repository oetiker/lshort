...
1977 to explore the potential of the digital printing equipment. These new devices were
beginning to infiltrate the publishing industry at that time. His goal was to reverse the trend of deteriorating typographical
...
\hologo{LuaTeX} from the start, to avoid complications down the road as you explore
...
New \TeX users may miss the whitespace
...
illustrate that, let's see how \csi{emph} behaves when used inside the text is
already in cursive
\begin{example}
  \itshape%!hide
Are you gonna eat \emph{that}?
...
produced output. Along with the package introduction we will also point
