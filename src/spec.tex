...
this is were they shine. Still, it is possible to use them for creating
presentations---while they will lack fancy animations and transitions (they are
still just a PDF file after all), you will be able to tap into other strengths
of \LaTeX{} ecosystem such as logical markup, math typesetting and capabilities
enabled by various packages.

Historically there have been several ways to create presentations, for example
the standard \LaTeX{}'s \cli{slides} class or the \pai{powerdot} package. While
these are still supported, this booklet will focus on the \pai*{beamer} package
that is most widely used today. We will only scratch the possibilities enabled
by the package, for a more in-depth tutorial please see the User Guide
distributed with it.
...
which denotes a single \enquote{slide} of your presentation.
\begin{example}
%!showbegin !hide
...
the curly braces these are actually optional arguments and has been
...
\end{document} %!hide
\end{example}
The title and subtitle may be also specified using \csi{frametitle} and
\csi{framesubtitle} commands.

Similarly to an article you can specify title, author and date of presentation
using the \csi{title}, \csi{author} and \csi{date} commands. The
\csi{maketitle} may then be used to create a title frame.
\begin{example}
\author{Jane Doe}
...
available for a richer title frame. The \csi{date} is often repurposed to hold
the conference name, since it is more informative than the exact date. Some
themes (discussed later) display these fields in each slide. If you find that
they are too long for such use, you may specify shorter versions via optional
argument like this:
\begin{minted}{latex}
  \title[Short Title]{A Title That Is Too Long}
\end{minted}

The sectioning commands, such as \csi{section}, are meant to be used between
frames and define a structure of presentation. Using them has no visible impact
on the frames themselves, but it will create nice \csi{tableofcontents}
output that will be properly hyperlinked.
\begin{example}
\begin{document} %!hide
\begin{frame}{Outline}
  \tableofcontents
...
In addition to the \csi{emph} command, \cli{beamer} provides additional means
of emphasis within the slide: the \csi{alert} command typesets its argument
using a bright red colour, while the \ei{block} environment displays its
contents with a title and separated from the rest of the text.
\begin{example}
\begin{document} %!hide
\begin{frame} %!hide
...
\end{document} %!hide
\end{example}
Many \pai{amsmath}\footnote{\pai{amsmath} is loaded automatically by
  \cli{beamer}.} environments, such as \ei{theorem} or \ei{proof} are redefined
...
So far, the created frames are very static---they show all of their contents at
...
rest of the frame is present on the next page.
\begin{example}[
  vertical_mode,
  to_page=3,
  paperheight=2.8cm,
...
so these can be mixed together as desired, though care is required to properly
set the slide numbers correctly.
...
\pai{beamer} to support them. Refer to its User Guide for more examples.
...
\end{document} %!hide
\end{example}
There are other possible arguments to this commands. For example, the
...
relevant if you use high-resolution display to present it. You can pass
...
More granular customisation is also possible using
\begin{lscommand}
  \csi{setbeamerfont}[element: m, attributes: m] \\
  \csi{setbeamercolor}[element: m, attributes: m]
\end{lscommand}
commands. The \carg{element} argument is the element for which the customised
font or colour should apply, while the \carg{attributes} specify the attributes
such as \cargv{size}, \cargv{shape} or \cargv{family} for fonts, and
...
projector supports that, you may prefer to change aspect ratio to a wider one.
You may do so by passing the \cargv{aspectratio} to the \cs{documentclass}
options. It accepts a single integer that encodes the desired ratio, for
example \cargv{169} is interpreted as \(16:9\), while \cargv{54} is interpreted
as \(5:4\). Wider ratios are especially useful if you want to include a sidebar
with table of contents on your slides.
