...
that the content before the pause is presented on the previous page, while the
rest of the frame is presented on the next page. These partial frames are called
slides.
%$ consider 'previous' and 'current', or 'current' and 'next', but maybe not
%$ 'previous' and 'next' since that leaves the current page blank.
\begin{example}[
  vertical_mode,
  to_page=3,
  paperheight=2.8cm,
...
switches to \cargv{handout} mode. %$ repetition (of previous paragraph)
...
slide that is not included in the compiled document. An example of doing so
...
\end{listing}
\endgroup
%$ Will this listing not fit side-by-side?
%$ It occupies an entire A4 page in its current form.
...
%$ Paragraph needs some work; unclear what is being said.
%$ You only get the <mode> slides when producing output of <mode>?
