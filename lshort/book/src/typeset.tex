...
Mongolian Script. In order to access the commands of Mon\TeX{}, the
package must be loaded in the document preamble:
\begin{lscommand}
\ci{usepackage}\verb|[|\emph{language},\emph{encoding}\verb|]{mls}|
\end{lscommand}
\noindent You can choose the \emph{language} option \pai{xalx} which issues
all captions and the date in Modern Mongolian, and the \emph{encoding} option
\iei{utf8}. To write a complete document in the traditional Mongolian script
...
Mongolian Cyrillic script is supported by \pai{babel} and you can load
Mongolian language support which enables hyphenation and changes all
automatic text to Mongolian with the following command:
...
\noindent where \iei{mn} is the \iei{cp1251} input encoding. You can also
invoke with \iei{utf8} instead.
